...
gp = gspread.service_account(filename='chiave_accesso.json')
gsheet = gp.open('Magazzino')
wsheet = gsheet.worksheet('Foglio1')

g = 100  # Grandezza icone  165

tipot = ['LAVATRICE', 'LAVATRICE C-ALTO', 'ASCIUGATRICE', 'LAVASTOVIGLIE', 'LAVASTOVIGLIE INCASSO', 'FORNO',
         'CUCINA GAS', 'CUCINA ELETTRICA', 'CONGELATORE', 'FRIGORIFERO', 'FRIGORIFERO INCASSO', 'CONDIZIONATORE',
         'CASSE AMPLIFICATE', 'MONOPATTINO', 'BICI', 'ALTRO']
tipo = deepcopy(tipot)
tipo.sort()
marche = deepcopy(wsheet.col_values(5)[1].split(', '))
marche.sort()
# ['BEKO', 'BOSCH', 'CANDY', 'SMEG', 'TREVI', 'WHIRLPOOL', 'ZEPHIR', ...]
...
BoxLayout:
    ...
    MDToolbar:
        id: barra
        right_action_items: [['immagini/cerca.png', lambda x: app.passa_cerca()], ['immagini/history.png', lambda x: app.passa_history()], ['immagini/agg.png', lambda y: app.aggiungi()]]
...
def find_differences():
    db_curr = wsheet.get_all_records()

    backgsheet = gp.open('Backup_precedente')
    backwsheet = backgsheet.worksheet('Foglio1')
    db_back = backwsheet.get_all_records()  #Lista di dizionari

    art_back = {d['ARTICOLO'] for d in db_back}
    added = [d for d in db_curr if d['ARTICOLO'] not in art_back]
    art_curr = {d['ARTICOLO'] for d in db_curr}
    removed = [d for d in db_back if d['ARTICOLO'] not in art_curr]
    common_with_curr_values = [d for d in db_curr if d['ARTICOLO'] in art_back]

    mod_res = []

    for row in common_with_curr_values:
        art = row['ARTICOLO']
        num_curr = row['NUMERO']
        num_back = next((elem for elem in db_back if elem['ARTICOLO'] == art), None)['NUMERO']
        if num_curr != num_back:
            if mod_res: mod_res.append(row)
            else: mod_res = [row]
            mod_res[-1]['Delta'] = num_curr - num_back

    return added, removed, mod_res
...
class App(MDApp):
    ...
    def prodotti(self, marca, tipo):
        ...
        for i in p:
            ...
            self.root.ids.prodotti.add_widget(
                TwoLineListItem(text=dati[int(i[r:])][2], secondary_text="Quantità: " + str(dati[int(i[r:])][3]),
                                on_press=lambda x, mar=dati[int(i[r:])][1], tip=dati[int(i[r:])][0],
                                                nome=dati[int(i[r:])][2], num=dati[int(i[r:])][3]: self.modifica(tip,
                                                                                                                 mar,
                                                                                                                 nome,
                                                                                                                 num)))
    ...
    def passa_history(self):
        self.root.ids.sc_man.current = 'history'
        self.scpr.append('history')
        self.root.ids.history.clear_widgets()

        added, removed, mod_res = find_differences()

        if len(mod_res) != 0:
            self.root.ids.history.add_widget(OneLineListItem(text='Articoli modificati', font_style="H6"))
            for row in mod_res:
                self.root.ids.history.add_widget(
                    TwoLineListItem(text=str(row['MARCA']) + ' - ' + str(row['ARTICOLO']),
                                    secondary_text="Quantità movimentata: " + str(row['Delta'])))
        if len(added) != 0:
            self.root.ids.history.add_widget(OneLineListItem(text='Articoli aggiunti', font_style="H6"))
            for row in added:
                self.root.ids.history.add_widget(
                    TwoLineListItem(text=str(row['MARCA']) + ' - ' + str(row['ARTICOLO']),
                                    secondary_text="Quantità presente: " + str(row['NUMERO'])))
        if len(removed) != 0:
            self.root.ids.history.add_widget(OneLineListItem(text='Articoli rimossi', font_style="H6"))
            for row in removed:
                self.root.ids.history.add_widget(
                    TwoLineListItem(text=str(row['MARCA']) + ' - ' + str(row['ARTICOLO']),
                                    secondary_text="Quantità presente: " + str(row['NUMERO'])))
    ...
    def cerca(self):
        ...
        for no in self.colonna(2):
            if c.replace(' ', '') == no.upper().replace(' ', '')[:len(c)] and no.upper() != 'ARTICOLO':
                s = 1
                l.append(no)
        l.sort()
        for no in l:
            ...
            self.root.ids.cerca_l.add_widget(TwoLineListItem(text=no, secondary_text="Quantità: " + nu,
                                                             on_press=lambda x, mar=ma, tip=ti, nome=no,
                                                                             num=nu: self.modifica(tip, mar, nome,
                                                                                                   num)))
        if s == 0:
